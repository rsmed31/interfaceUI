...
def map_layout(ip_locations):
    ...
    if not valid_locations:
        return html.Div(
            [
                ...
                html.P(
                    "No public IP addresses available for geolocation",
    ...
    valid_coords = [
        (loc["latitude"], loc["longitude"]) for loc in valid_locations.values()
    ]
    ...
    return html.Div(
        [
            ...
            html.Div(
                f"Displaying {len(markers)} locations", className="text-center mt-20"
            ),
        ],
        className="card",
    )
